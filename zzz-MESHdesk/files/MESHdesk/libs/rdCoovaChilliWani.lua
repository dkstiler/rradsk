...
function rdCoovaChilli:startPortals()
	...
	for i=1,self.cpCount do	
	        ...
	        if f~=nil then --File do exist; try to start this captive portal
	        	...
	        	self:__sleep(5)
	        	--Hack for PM-WANI                                                
                --os.execute("softflowd -i tun0 tcp port 443");
                --Collect ALL the flows
                os.execute("softflowd -i tun0");
...
function rdCoovaChilli.__doConfigs(self,p)
	...
	for k,v in ipairs(p)do 
		...
		if(true)then --FIXME HARD CODED ON
			...
			if(ip)then
				...
				self:__doPrivoxy(conf_file,ip,k);
			end
		end
...
function rdCoovaChilli.__doPrivoxy(self,conf_file,ip,number)
    --=============================
...
function rdCoovaChilli.__doPrivoxyZZ(self,conf_file,ip,proxy_ip,proxy_port,enc_string,number)
    --=============================
    --First we do the config file==
    --=============================
    local fp 	= io.open( conf_file, "r" )
