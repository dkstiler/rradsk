...
cp_config_file          = '/etc/MESHdesk/configs/captive_config.json';
...
function try_settings_through_lan() 
	...
	if(lan_config_fail)then	
		...
		if(mac_not_defined)then
		    log("MAC not known -> switch over to Auto Captive Portal over LAN ");
		    c:prepCaptiveConfig(dns_works); --Here we DO NOT specify the WiFi flag (optional)
		    configure_device(cp_config_file);
		end
	end
end
	...
	while(radio < radio_count) do
		...
		if(wifi_is_up)then
			...
			if(got_settings)then
				...
				configure_device(config_file)
				if(mac_not_defined)then
				    log("MAC not known -> switch over to Auto Captive Portal over WiFi "..radio);
				    local c = rdConfig();
		            c:prepCaptiveConfig(dns_works,true); --Here we specify the WiFi flag (optional, but required here)
		            configure_device(cp_config_file);
		        end
...
function configure_device(config)
    ...
    if(o.success == false)then --If the device was not yet assigned we need to give feedback about it
	    ...
	    if(o.error ~= nil)then
	        print(o.error);
	        log(o.error);
	        log("Setting the device in CP config mode")
	        mac_not_defined = true;
	        --try_controller_modes();
	        return;
	    end
	...
	--ext:startOne('/etc/MESHdesk/actions_checker.lua &','actions_checker.lua') --Sept 2019 No need for actions checker now part of heartbeat
    ...
    sleep(15) -- Wait a bit before doing this part else the DHCP not work correct FIXME 5 seemed to fcause issue made it 15
...
function ap_configure_device(config)
    ...
    -- Check if there are perhaps some captive portals to set up once everything has been done --
    sleep(15) -- Wait a bit before doing this part else the DHCP not work correct

    os.execute("/etc/init.d/firewall reload") --Activate the new firewall rules especially NAT to LAN
